 ...
 *      而是直接講明「沒有任何立場救得了它」。
 *   U4 試駕清單的另外兩台取自 scoring.rank(STATE.weights) 的前兩名，
 *      同一車系的不同等級可能同時上榜（例如 HR-V 的兩個 e:HEV 等級），本檔不做車系去重。
 *   U5 「資料的邊界」那一段用 IntersectionObserver 在捲到它時才靜止 0.7 秒後浮現；
 *      沒有 IntersectionObserver 的環境改用時間排程，30 秒後一定會顯示，不會被永遠藏住。
 */
